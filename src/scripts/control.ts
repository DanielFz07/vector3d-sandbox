...
document.getElementById("vmenu_form")?.addEventListener("submit", (e) =>
{
    ...
    values.length = Math.sqrt((Math.pow(sub.x, 2)+Math.pow(sub.y, 2)+Math.pow(sub.z, 2)))
    ...
    getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.origin.set(values.o_x,values.o_y,values.o_z);
    });
    getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.direction.set(values.d_x,values.d_y,values.d_z);
    });
    getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.setLength(values.length, optionalParams);
    });
    
    getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.updateGeo()
        });
...
document.getElementById("color_menu_form")?.addEventListener("submit", (e) =>
{
    ...
    getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.material.color.setHex(color_value);
    });
...
document.getElementById("position")?.addEventListener("click", (event) => 
{
    ...
    new FormData(document.getElementById("vmenu_form") as HTMLFormElement);

    document.getElementById("vo_xvalue")?.setAttribute('value', `${getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.origin.x
    })
    }`);
    document.getElementById("vo_yvalue")?.setAttribute('value', `${getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.origin.y
    })
    }`);
    document.getElementById("vo_zvalue")?.setAttribute('value', `${getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.origin.z
    })
    }`);
    document.getElementById("vd_xvalue")?.setAttribute('value', `${getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.direction.x
    })
    }`);
    document.getElementById("vd_yvalue")?.setAttribute('value', `${getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.direction.y
    })
    }`);
    document.getElementById("vd_zvalue")?.setAttribute('value', `${getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.direction.z
    })
    }`);
    document.getElementById("vv_module")?.setAttribute('value', `${getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) obj.geometry.length
    })
    }`);
...
document.getElementById("color")?.addEventListener("click", (event) => 
{
    ...
    document.getElementById("color_value")?.setAttribute('value', `0x${getPrimarySelect().traverse( (obj) => {
        if ( obj instanceof THREE.Mesh ) return obj.material.color.getHex() }) }`);
